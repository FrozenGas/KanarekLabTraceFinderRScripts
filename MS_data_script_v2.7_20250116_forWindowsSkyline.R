...
#
# Description:
# This script takes input of the combined data matrix from Skyline and normalizes by internal standards and total polars.
#
# CSV outputs:
...
print("Where is the working directory?  (e.g.: /Documents/MS Data/202201208 (can use quotation marks)")
...
print("what is the name of the input .csv?")
filename <- readline(prompt="filename:")

# Extract and normalize raw data and pools/dilutions to standards ----------------------------------------------------------------

print("reading data .csv...")
rawdata <- read.csv(paste(filename, sep=""), row.names = 1) #read csv
rawdata[rawdata == "N/F"] <- NA #replace NF with NA
rawdata[rawdata == 0] <- NA #replace 0 with NA
...
rawdata <- t(rawdata) #flip the columns and rows (transpose)
View(rawdata) #print rawdata just to confirm

print(" data is now displayed.")
...
if (input == "y")
{
  ...
  while(i == 0)
  {
    ...
    {
      temp <- grepl(rowstoremove[i],rownames(rawdata))
      if (i == 1)
      {
        rowlogical <- temp
      } else {
    ...
    rows <- rawdata[as.logical(rowlogical)]
    
    print(rownames(rawdata)[as.logical(rowlogical)])
...
#if the Rsq is NA, replace it with 0 (fail)

Rsq[is.na(Rsq)] <- 0

#if the CV is NA, replace it with 1 (fail)
...
Rsqval <- readline(prompt="What cut-off should be used for Rsq? (e.g. 0.95):")
CVval <- readline(prompt="What cut-off should be used for CV? (e.g. 0.30):")

write.csv(c(Rsqval, CVval),file=paste(outputpath,"Rsq_CV.csv", sep=""))
...
{
  if ((filtered_sampledata[(nrow(filtered_sampledata)-1),i]<Rsqval)|(filtered_sampledata[(nrow(filtered_sampledata)-0),i]>CVval)) #filter out if Rsq <0.949 or CV > 0.301
  {
...
filtered_sampledata <- read.csv(paste(outputpath,"RsqCVfiltered_normalized_sampledata.csv", sep=""), row.names = 1)
